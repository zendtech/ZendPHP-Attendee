...
# Required software
FCGI_CMD_PATH=$(command -v cgi-fcgi) || { >&2 echo "Make sure 'fcgi' package is installed. Aborting."; exit 4; }
command -v sed 1> /dev/null || { >&2 echo "Make sure sed is installed. Aborting."; exit 4; }
command -v tail 1> /dev/null || { >&2 echo "Make sure tail is installed. Aborting."; exit 4; }
command -v grep 1> /dev/null || { >&2 echo "Make sure grep is installed. Aborting."; exit 4; }
...
check_fpm_health_by() {
    ...
    if test "${VERBOSE}" = 1;then
        printf "'%s' value '%s' and expected is less than '%s'\\n" "${OPTION}" "${VALUE_ACTUAL}" "${VALUE_EXPECTED}";
    fi
...
if ! GETOPT=$(getopt -o v --long verbose,accepted-conn:,listen-queue:,max-listen-queue:,listen-queue-len:,idle-processes:,active-processes:,total-processes:,max-active-processes:,max-children-reached:,slow-requests: -n 'php-fpm-healthcheck' -- "$@"); then
    >&2 echo "Invalid options, terminating." ;
    exit 3;
fi
